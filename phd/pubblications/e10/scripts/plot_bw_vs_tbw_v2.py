...
if __name__ == "__main__":
    ...
    bw = filesize / (datafile.write + datafile.close)
    tbw = filesize / datafile.write
    ...
    lns1 = ax1.bar(x-1.5*width, bw[0:20], width, color='r', label='BW Cache Disable', edgecolor='black', log='true')
    lns2 = ax1.bar(x-0.5*width, bw[20:40], width, color='g', label='BW Cache Enable', edgecolor='black', log='true')
    lns3 = ax1.bar(x+0.5*width, tbw[20:40], width, color='b', label='TBW Cache Enable', edgecolor='black', log='true')

    # add yticks for logarithmic scale
    ylabels = [10, 100, 1000, 1000, 10000]
