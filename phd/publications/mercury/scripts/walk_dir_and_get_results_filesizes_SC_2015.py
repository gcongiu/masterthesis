...
if __name__ == '__main__':
    ...
    if 'GPFS' in arg_of_program.dir:  # or 'Lustre' in arg_of_program.dir:
        list_of_list_AM_false_reads_client = [[] for i in range(0, len(list_of_input_filenames))]
        list_of_list_AM_false_readbytes_client = [[] for i in range(0, len(list_of_input_filenames))]
    ...
    if 'GPFS' in arg_of_program.dir:  # or 'Lustre' in arg_of_program.dir:
        list_of_list_AM_true_reads_client = [[] for i in range(0, len(list_of_input_filenames))]
        list_of_list_AM_true_readbytes_client = [[] for i in range(0, len(list_of_input_filenames))]
        ...
        if 'False' in directory.split("-")[1].split("_"):
            ...
            for inputfile in list_of_input_filenames:
                if inputfile in directory.split("-")[3].split("."):
                    ...
                    if 'GPFS' in arg_of_program.dir:  # or 'Lustre' in arg_of_program.dir:
                        list_of_list_AM_false_reads_client[index_in_list].append(read_reads_client_info(disk_stat_file))
                        list_of_list_AM_false_readbytes_client[index_in_list].append(read_read_bytes_client(disk_stat_file))
        ...
        if 'True' in directory.split("-")[1].split("_"):
            ...
            for inputfile in list_of_input_filenames:
                if inputfile in directory.split("-")[5].split("."):
                    ...
                    if 'GPFS' in arg_of_program.dir:  # or 'Lustre' in arg_of_program.dir:
                        list_of_list_AM_true_reads_client[index_in_list].append(read_reads_client_info(disk_stat_file))
                        list_of_list_AM_true_readbytes_client[index_in_list].append(read_read_bytes_client(disk_stat_file))
    ...
    for j in L2:
        ...
        if 'GPFS' in arg_of_program.dir:  # or 'Lustre' in arg_of_program.dir:
            mean_reads_AMfalse_client = evaluate_mean(list_of_list_AM_false_reads_client[index])
            std_reads_AMfalse_client = evaluate_std(list_of_list_AM_false_reads_client[index])

            mean_readbytes_AMfalse_client = evaluate_mean(list_of_list_AM_false_readbytes_client[index])
            std_readbytes_AMfalse_client = evaluate_std(list_of_list_AM_false_readbytes_client[index])
        ...
        if 'GPFS' in arg_of_program.dir:  # or 'Lustre' in arg_of_program.dir:
            dict_with_results["AM_False"]["%s" % j]["reads_client"] = mean_reads_AMfalse_client    ## get's runtime
            dict_with_results["AM_False"]["%s" % j]["std_reads_client"] = std_reads_AMfalse_client
            dict_with_results["AM_False"]["%s" % j]["readbytes_client"] = mean_readbytes_AMfalse_client    ## get's runtime
            dict_with_results["AM_False"]["%s" % j]["std_readbytes_client"] = std_readbytes_AMfalse_client
        ...
        if 'GPFS' in arg_of_program.dir:  # or 'Lustre' in arg_of_program.dir:
            ...
            mean_reads_AMtrue_client = evaluate_mean(list_of_list_AM_true_reads_client[index])
            std_reads_AMtrue_client = evaluate_std(list_of_list_AM_true_reads_client[index])

            mean_readbytes_AMtrue_client = evaluate_mean(list_of_list_AM_true_readbytes_client[index])
            std_readbytes_AMtrue_client = evaluate_std(list_of_list_AM_true_readbytes_client[index])
        ...
        if 'GPFS' in arg_of_program.dir:  # or 'Lustre' in arg_of_program.dir:
            dict_with_results["AM_True"]["%s" % j]["reads_client"] = mean_reads_AMtrue_client    ## get's runtime
            dict_with_results["AM_True"]["%s" % j]["std_reads_client"] = std_reads_AMtrue_client
            dict_with_results["AM_True"]["%s" % j]["readbytes_client"] = mean_readbytes_AMtrue_client    ## get's runtime
            dict_with_results["AM_True"]["%s" % j]["std_readbytes_client"] = std_readbytes_AMtrue_client
