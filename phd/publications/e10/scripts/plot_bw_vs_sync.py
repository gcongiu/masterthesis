...
if __name__ == "__main__":
    ...
    filesize = long(options.filesize)
    ...
    sync = (datafile.shuffle_all2all + datafile.post_write) / (datafile.shuffle_write + datafile.write + datafile.post_write) * 100
    lns2 = ax2.plot(x, sync[0:25], linestyle='--', color='r', marker='o', alpha=0.7, label='Global Synchronisation')
    ...
    ax2 = ax1.twinx()
    lns2 = ax2.plot(x, sync[25:50], linestyle='--', color='r', marker='o', alpha=0.7, label='Global Synchronisation')
    ax2.set_ylabel('io time %')
    lns = lns1 + lns2
    labs = [l.get_label() for l in lns]
    plt.legend(lns, labs, bbox_to_anchor=(0., 1.02, 1., .102), loc=3, ncol=2, mode='expand', borderaxespad=0.)
